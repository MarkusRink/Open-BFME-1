...
// partial score=0.994 date=2026-09-08
...
// and its random ColorScale value.
class File {};
...
StreamWriter *formatColor(StreamWriter *stream, const float *value);
extern const float BfmeZeroRange;

namespace FXParticleSystem {
void writeDefaultColorHeader(const void *self, File *file, unsigned int *flags);
...
{
	writeDefaultColorHeader(this, &file, &flags);
	...
	for (unsigned int keyIndex = 0; keyIndex < 8; ++keyIndex, ++key) {
		if (key->red != BfmeZeroRange || key->green != BfmeZeroRange ||
			...
			writer->append(" = ");
			writer = formatColor(reinterpret_cast<StreamWriter *>(&stream), &key->red);
			writer->separate(' ');
			writer = formatInteger(writer, key->frame);
			writer->separate('\n');
		}
	}

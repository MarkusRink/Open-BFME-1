...
class BfmeMeshModelResetShim
{
public:
	void Reset(int polycount,int vertcount,int passcount,int skin);
};
...
//	WWDEBUG_SAY(("Note: Mesh %s was never used\n",Get_Name()));
	...
	if (renderer != NULL) {
		renderer->Unregister_Mesh_Type(this);
	}

	int skin = (int)((unsigned int)Flags >> 10);
	skin &= (int)0xFFFFFF01;
	((BfmeMeshModelResetShim *)this)->Reset(0,0,0,skin);
	REF_PTR_RELEASE(MatInfo);

	if (DefMatDesc != NULL) {
		delete (BfmeHolderBY *)DefMatDesc;
	}
	if (AlternateMatDesc != NULL) {
		delete (BfmeHolderBY *)AlternateMatDesc;
	}
	return ;
}

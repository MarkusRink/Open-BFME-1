// ?bfmeDropOwned@Gen_00379250@@QAEXXZ
...
{
public:
	BfmeElemAB **bfmeBegin(void) const
	{
		return m_bfmeStart;
	}

	BfmeElemAB **bfmeEnd(void) const
	{
		return m_bfmeFinish;
	}

	void bfmeErase(BfmeElemAB **position)
	{
		BfmeElemAB **next = position + 1;
...
{
public:
	void bfmeDropOwned(void);

private:
	int m_bfmeHead[2];
	BfmeElemAB **m_bfmeStart;
	BfmeElemAB **m_bfmeFinish;
};

// ?bfmeDropOwned@Gen_00379250@@QAEXXZ
void Gen_00379250::bfmeDropOwned(void)
{
	register BfmeElemAB **next;
	register BfmeElemAB **it = m_bfmeStart;
	register BfmeElemAB **finish = m_bfmeFinish;
	if (it == finish)
		return;
	...
	while (it != m_bfmeFinish)
	{
			...
			if (element != 0)
			{
				...
				{
					if (element->m_bfmeRef != 0)
						element->m_bfmeRef = element->m_bfmeRef->bfmeRelease();

					++it;
					++next;
					goto decrement_finish;
				}
			}

			if (next != m_bfmeFinish)
				memmove(it, next, (char *)m_bfmeFinish - (char *)next);

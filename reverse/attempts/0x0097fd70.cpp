// ?Build_Definition@ParticleEmitterClass@@QBEPAVParticleEmitterDefClass@@XZ
...
// BFME's emitter path obtains the texture name through the companion object
// at this emitter's subobject, rather than through ParticleBufferClass's
// TextureHandleClass accessor.  Keep these ABI-only views local to this TU;
// the surrounding WW3D headers describe the Zero Hour layout and do not own
// the BFME conversion helper's opaque string object.
class BfmeOtherDQF
{
};
...
class BfmeParticleBufferFields
{
public:
	...
	float AccelY;
	float AccelZ;
	unsigned char m_padEC[4];
	unsigned int MaxAge;
	unsigned int FutureStartTime;
	unsigned char m_padF8[0x40];
	unsigned int NumColorKeyFrames;
	unsigned int *ColorKeyFrameTimes;
	unsigned char m_pad140[0xc0];
	float InitialOrientationRandom;
};

class BfmeStringScratch
{
public:
	char *m_buffer;
	BfmeStringScratch &bfmeGetString(BfmeStringScratch &out);
	void Free_String(void);
};

#pragma comment(linker, "/alternatename:?bfmeGetString@BfmeStringScratch@@QAEAAV1@AAV1@@Z=?d_008ff5b0@@YAXXZ")
#pragma comment(linker, "/alternatename:?Free_String@BfmeStringScratch@@QAEXXZ=?Free_String@StringClass@@AAEXXZ")
...
// ?ParticleEmitterClass::Build_Definition present-unmatched
...
{	
	...
	if (pdefinition != NULL) {
		...
		// Set the texture's filename.  BFME returns an opaque StringClass-like
		// object through the emitter's conversion subobject.  The helper fills
		// the scratch object and returns it by reference; both objects are
		// released exactly as the retail path does.
		BfmeStringScratch *texture_name;
		((BfmeThingDQF *)this)->bfmeGoDQF ((BfmeOtherDQF *)&texture_name);
		if (texture_name != NULL) {
			BfmeStringScratch texture_string;
			pdefinition->Set_Texture_Filename (
				((BfmeStringScratch *)&texture_name)->bfmeGetString (texture_string).m_buffer);
			texture_string.Free_String ();
		}

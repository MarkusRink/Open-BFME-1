...
// partial score=0.995 date=2026-09-06
// cl: /DNDEBUG /MD /EHsc /D_STLP_USE_STATIC_LIB
// stlport
// Open-BFME5: clean C++ reconstruction of the retail module-data constructor.
...
// The class remains four bytes wide, as the destructor and field table show.
class AsciiString
{
public:
...
// This array element has the retail order, AsciiString followed by the
// particle-system pointer.  Its implicit assignment and destruction produce
// the two calls in the fxbones.clear() erase path.
...
// that __ehvec_ctor records for the three OCL members at +0x6c.
	...
	FXBoneInfo *last)
{
	...
	while (count > 0)
	{
		...
		const ParticleSystemTemplate *particleTemplate =
			source->particleSystemTemplate;
		destination->particleSystemTemplate = particleTemplate;
		int nextCount = count;
		++source;
		++destination;
		count = nextCount - 1;
...
private:
	...
	float m_bfmeUnknown44;                          // +0x44, retail default 0.06f
	...
	float m_bfmeUnknownB4;                          // +0xb4, retail default -9.876540f
};

// ??0StructureToppleUpdateModuleData@@QAE@XZ
StructureToppleUpdateModuleData::StructureToppleUpdateModuleData()
{
	register StructureToppleUpdateModuleData *self = this;
	(void)self;
	m_minToppleDelay = 0;
	m_maxToppleDelay = 0;
	...
	StructureToppleFXBoneVectorLayout &bones =
		*(StructureToppleFXBoneVectorLayout *)&fxbones;
	StructureToppleAngleVectorLayout &angles =
		*(StructureToppleAngleVectorLayout *)&angleFX;
	eraseStructureToppleFXBoneRange(bones, bones.m_begin, bones.m_finish);
	eraseStructureToppleAngleRange(angles, angles.m_begin, angles.m_finish);
}

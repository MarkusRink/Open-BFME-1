...
// GiantBirdSlowDeathBehavior has no header anywhere in this tree, but the
// identity of the retail factory at 0x001157D0 is not in doubt: that body pushes
// 0xD8 to the global operator new and then calls the constructor functions.csv
// already matches at 0x001FFB40 as
// ??0GiantBirdSlowDeathBehavior@@QAE@PAVThing@@PBVModuleData@@@Z. The row that
// used to hold the address named emit_TransitionDamageFX, which constructs a
// different class; the constructor displacement was the only byte that ever
// differed. This TU-scoped declaration carries the size and the constructor
// signature and invents no members, so the factory can be emitted from the name
// the ledger's own alias row already gives it.
...
Module *emit_GiantBirdSlowDeathBehavior( Thing *thing, const ModuleData *moduleData )
{
	return (Module *)::new GiantBirdSlowDeathBehavior( thing, moduleData );
}

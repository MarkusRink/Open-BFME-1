...
static char enc1[16] =      {'\0','a','m','e','S','p','y','3','D','\0','\0','\0','\0','\0','\0','\0'};
static char enc2[16]=       {'\0','n','d','u','s','t','r','i','e','s','\0','\0','\0','\0','\0','\0'};
static char enc3[16] =      {'\0','r','o','j','e','c','t','A','p','h','e','x','\0','\0','\0','\0'};
static char statsfile[16] = {'\0','s','t','a','t','s','.','d','a','t','\0','\0','\0','\0','\0','\0'};
static char finalstr[10] =  {'\0','f','i','n','a','l','\\','\0','\0','\0'};
...
static gsi_time inittimeout = 10000;  // 10 seconds
...
/****************************************************************************/
#define RAWSIZE 128
char *GenerateAuthA(char *challenge,  char *password, char response[33])
{
	char rawout[RAWSIZE];
...
#ifdef GSI_UNICODE
char *GenerateAuthW(char* challenge,  unsigned short *password, char response[33])
{
	...
	case init_awaitchallenge:
		{
			...
			#ifdef SN_SYSTEMS
				ret = (int)recv(sock, rcvbuffer+rcvlen, rcvmax-rcvlen, 0);
			#else
				ret = (int)recv(sock, rcvbuffer+rcvlen, (unsigned int)rcvmax-rcvlen, 0);
			#endif
			rcvlen += ret;
			rcvmax -= ret;
...
/****************************************************************************/
void PreAuthenticatePlayerCDA(int localid, char *nick, char *keyhash,  char *challengeresponse, PersAuthCallbackFn callback, void *instance)
{
...
#ifdef GSI_UNICODE
void PreAuthenticatePlayerCDW(int localid, unsigned short*nick, char *keyhash,  char *challengeresponse, PersAuthCallbackFn callback, void *instance)
{
...
/****************************************************************************/
void GetProfileIDFromCDA(int localid, char *nick, char *keyhash, ProfileCallbackFn callback, void *instance)
{
...
#ifdef GSI_UNICODE
void GetProfileIDFromCDW(int localid, unsigned short *nick, char *keyhash, ProfileCallbackFn callback, void *instance)
{
	char* nick_A = UCS2ToUTF8StringAlloc(nick);
...
/****************************************************************************/
void SetPersistDataValuesA(int localid, int profileid, persisttype_t type, int index, char *keyvalues, PersDataSaveCallbackFn callback, void *instance)
{
	SetPersistDataHelper(localid, profileid, type, index, keyvalues, (int)strlen(keyvalues) + 1, callback, instance, 1);
}
#ifdef GSI_UNICODE
void SetPersistDataValuesW(int localid, int profileid, persisttype_t type, int index, unsigned short *keyvalues, PersDataSaveCallbackFn callback, void *instance)
{
...
int PersistThink()
{
	...
	while (SocketReadable(sock))
	{
		...
		#ifdef SN_SYSTEMS
			len = (int)recv(sock, rcvbuffer + rcvlen, rcvmax - rcvlen, 0);		
		#else
			len = (int)recv(sock, rcvbuffer + rcvlen, (unsigned int)rcvmax - rcvlen, 0);
		#endif
...
static int DoSend(char *data, int len)
{
	...
	while(sent < (len+7))
	{
		// Send remaining data
		#ifdef SN_SYSTEMS
			int ret = (int)send(sock, (data+sent), (len+7-sent), 0);
		#else
			int ret = (int)send(sock, (data+sent), (unsigned int)(len+7-sent), 0);
		#endif
		if (ret == SOCKET_ERROR)
		{
			int anError = GOAGetLastError(sock);

...
// Body in GameWindowManagerScript_parseDrawData.asm (exact 1221B retail).
Bool parseDrawData( char *token, WinInstanceData *instData,
 char *buffer, void *data ); // defined in .asm
...
// ?parseWindow@@YAPAVGameWindow@@PAVFile@@PAD@Z matched 1328 bytes (Open-BFME5)
__declspec(naked) static GameWindow *parseWindow(File *, char *)
{
	__asm
	{
		__emit 0x6a;
...
// Retail destroys the four strings, it does not clear them: four plain calls to
// the AsciiString destructor at 0x00887940, the last one a tail jump. The donor
// could only reach them as one anonymous array at 0x012F2570 and said so -- "the
// four immediates prove their adjacency, not what BFME calls them". This file
// names them, and the four addresses fall out in declaration order, so the fold
// puts real names on four globals the image does not name.
void GameWindowManager::freeStaticStrings(void)
{
	theSystemString.~AsciiString();
	theInputString.~AsciiString();
	theTooltipString.~AsciiString();

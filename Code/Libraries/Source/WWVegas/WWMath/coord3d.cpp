...
{
    // Preserve the original x87 load order for the two shared operands.
    x = left.y * right.z - left.z * *(const volatile float *)&right.y;
    y = left.z * *(const volatile float *)&right.x - left.x * right.z;

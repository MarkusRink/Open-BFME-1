...
// Open-BFME: STLport _Rb_tree<Gen_p16pod, Gen_p16pod>::_M_insert, the
// four-argument form, at retail 0x00927C30 -- a set-shaped tree (KeyOfValue
// is identity, not _Select1st<pair<..>>): node = 16-byte base + 16-byte
// value, so the byte pushed to the allocator is 0x20.  Gen_p16pod is the
// same 16-byte POD tag already used by Code/gen_small/tgrid_*.cpp, so the
// _Construct call resolves to the already-landed
// ??$_Construct@UGen_p16pod@@U1@@_STL@@YAXPAUGen_p16pod@@ABU1@@Z symbol
// (retail 0x009267A0, tg_009267a0) without a new pin.  The comparator calls
// the still-unconverted dump at 0x00927600, so it needs a fresh pin.

struct BfmeRbTreeInsert00927C30AnchorHelper;
...
private:
	...
	friend struct ::BfmeRbTreeInsert00927C30AnchorHelper;
};
...
// reusing this tag matches the already-landed Construct instantiation for
...
// retail 0x00927C30
Rva00927C30Tree::iterator BfmeRbTreeInsert00927C30( Rva00927C30Tree *tree,
	_STL::_Rb_tree_node_base *x, _STL::_Rb_tree_node_base *y,
	const Gen_p16pod &v, _STL::_Rb_tree_node_base *w )
{
	return BfmeRbTreeInsert00927C30AnchorHelper::run( tree, x, y, v, w );
}

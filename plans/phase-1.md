...
- Follow plans/README.md; record the execution start SHA/time and twelve-hour
...
- `python3 tools/rename_addressed_sources.py --under Code/GameEngine/Source/Common --limit 8`
...
- Each worker proves its root can run a scoped build on its first assigned TU.
  ...
  in plans/cleanup-run.md. Reforecast if setup consumes the intended pilot window.
